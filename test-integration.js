...
async function runTests() {
    ...
    // Ensure we give the filesystem a moment if init just ran
    ...
    try {
        ...
        if (typeof dora.successRate !== 'number') throw new Error('Invalid successRate type');
    ...
    try {
        ...
        const quality = await analytics.getQualityMetrics(1); // Test with dummy ID
        if (typeof quality.pass_rate !== 'number') throw new Error('Invalid pass_rate type');
